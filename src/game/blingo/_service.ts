...
export const blingoService = {
    handleAction : async ( actionParams:any ) => {
        ...
        if( action === "spin" || action === "chooseCell" ) {
            ...
            if( userInfo.gameStatus.purCount>=0 || userInfo.gameStatus.isExtra ) {
                isExtra = true;
                userInfo.gameStatus.cells.forEach((sym:number, ind:number) => {
                    if( userInfo.gameStatus.symbols.includes( sym ) && userInfo.gameStatus.gameMatches.includes( ind ) ) isExtra = false;
                })
            ...
            if( action === "spin" ) {
                ...
                if( userInfo.gameStatus.spinsRemaining===0 ) {
                    ...
                    console.log(`&& service purCount=${ userInfo.gameStatus.purCount }`)
                    if( userInfo.gameStatus.purCount>=1 && userInfo.gameStatus.purCount<=5 ) {
                        userInfo.gameStatus.fspSpinsRemaining--;
                    }
                    if( userInfo.gameStatus.purCount>5 ) {
                        userInfo.gameStatus.isExtra = false;
                        userInfo.gameStatus.fpSpinsRemaining--;
                    }
                }
